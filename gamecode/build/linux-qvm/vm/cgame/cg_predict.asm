...
ASGNI4
line 976
;976:	cg_pmove.grapplePull = cgs.g_grapplePull;
...
line 996
;977:
;978:	// Like the comments described above, a player's state is entirely
;979:	// re-predicted from the last valid snapshot every client frame, which
;980:	// can be really, really, really slow.  Every old command has to be
;981:	// run again.  For every client frame that is *not* directly after a
;982:	// snapshot, this is unnecessary, since we have no new information.
;983:	// For those, we'll play back the predictions from the last frame and
;984:	// predict only the newest commands.  Essentially, we'll be doing
;985:	// an incremental predict instead of a full predict.
;986:	//
;987:	// If we have a new snapshot, we can compare its player state's command
;988:	// time to the command times in the queue to find a match.  If we find
;989:	// a matching state, and the predicted version has not deviated, we can
;990:	// use the predicted state as a base - and also do an incremental predict.
;991:	//
;992:	// With this method, we get incremental predicts on every client frame
;993:	// except a frame following a new snapshot in which there was a prediction
;994:	// error.  This yeilds anywhere from a 15% to 40% performance increase,
;995:	// depending on how much of a bottleneck the CPU is.
;996:	if( 1 /* cg_optimizePrediction.integer */ ) {
line 997
;997:		if( cg.nextFrameTeleport || cg.thisFrameTeleport ) {
ADDRGP4 cg+107596
INDIRI4
CNSTI4 0
NEI4 $750
...
LABELV $750
line 999
;998:			// do a full predict
;999:			cg.lastPredictedCommand = 0;
ADDRGP4 cg+126112
CNSTI4 0
ASGNI4
line 1000
;1000:			cg.stateTail = cg.stateHead;
ADDRGP4 cg+157012
ADDRGP4 cg+157008
INDIRI4
ASGNI4
line 1001
;1001:			predictCmd = current - CMD_BACKUP + 1;
ADDRLP4 484
ADDRLP4 476
INDIRI4
CNSTI4 64
...
LABELV $746
line 1005
;1003:		// cg.physicsTime is the current snapshot's serverTime if it's the same
;1004:		// as the last one
;1005:		else if( cg.physicsTime == cg.lastServerTime ) {
ADDRGP4 cg+107612
INDIRI4
ADDRGP4 cg+126116
INDIRI4
NEI4 $754
line 1007
;1006:			// we have no new information, so do an incremental predict
;1007:			predictCmd = cg.lastPredictedCommand + 1;
ADDRLP4 484
ADDRGP4 cg+126112
INDIRI4
CNSTI4 1
...
LABELV $754
line 1012
;1009:			// we have a new snapshot
;1010:			int i;
;1011:			int errorcode;
;1012:			qboolean error = qtrue;
ADDRLP4 552
CNSTI4 1
ASGNI4
line 1015
;1013:
;1014:			// loop through the saved states queue
;1015:			for( i = cg.stateHead; i != cg.stateTail; i = ( i + 1 ) % NUM_SAVED_STATES ) {
ADDRLP4 544
ADDRGP4 cg+157008
INDIRI4
ASGNI4
...
LABELV $759
line 1018
;1016:				// if we find a predicted state whose commandTime matches the snapshot
;1017:				// player state's commandTime
;1018:				if( cg.savedPmoveStates[ i ].commandTime != cg.predictedPlayerState.commandTime ) {
ADDRLP4 544
INDIRI4
CNSTI4 468
MULI4
...
line 1022
;1020:				}
;1021:				// make sure the state differences are acceptable
;1022:				errorcode = CG_IsUnacceptableError( &cg.predictedPlayerState, &cg.savedPmoveStates[ i ], &moved );
ADDRGP4 cg+107636
ARGP4
ADDRLP4 544
INDIRI4
...
EQI4 $771
line 1024
;1024:					if( cg_showmiss.integer > 1 )
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 1
LEI4 $761
line 1025
;1025:						CG_Printf( "errorcode %d at %d\n", errorcode, cg.time );
ADDRGP4 $776
ARGP4
ADDRLP4 548
INDIRI4
...
;1027:				}
;1028:  
;1029:				// this one is almost exact, so we'll copy it in as the starting point
;1030:				*cg_pmove.ps = cg.savedPmoveStates[ i ];
ADDRGP4 cg_pmove
INDIRP4
ADDRLP4 544
INDIRI4
...
ASGNB 468
line 1032
;1031:				// advance the head
;1032:				cg.stateHead = ( i + 1 ) % NUM_SAVED_STATES;
ADDRGP4 cg+157008
ADDRLP4 544
INDIRI4
CNSTI4 1
...
line 1035
;1033:  
;1034:				// set the next command to predict
;1035:				predictCmd = cg.lastPredictedCommand + 1;
ADDRLP4 484
ADDRGP4 cg+126112
INDIRI4
CNSTI4 1
...
line 1038
;1036:  
;1037:				// a saved state matched, so flag it
;1038:				error = qfalse;
ADDRLP4 552
CNSTI4 0
ASGNI4
...
;1040:			}
;1041:
;1042:			// if no saved states matched
;1043:			if ( error ) {
ADDRLP4 552
INDIRI4
CNSTI4 0
EQI4 $781
line 1045
;1044:				// do a full predict
;1045:				cg.lastPredictedCommand = 0;
ADDRGP4 cg+126112
CNSTI4 0
ASGNI4
line 1046
;1046:				cg.stateTail = cg.stateHead;
ADDRGP4 cg+157012
ADDRGP4 cg+157008
INDIRI4
ASGNI4
line 1047
;1047:				predictCmd = current - CMD_BACKUP + 1;
ADDRLP4 484
ADDRLP4 476
INDIRI4
CNSTI4 64
...
LABELV $747
line 1052
;1050:		// keep track of the server time of the last snapshot so we
;1051:		// know when we're starting from a new one in future calls
;1052:		cg.lastServerTime = cg.physicsTime;
ADDRGP4 cg+126116
ADDRGP4 cg+107612
INDIRI4
ASGNI4
line 1053
;1053:		stateIndex = cg.stateHead;
ADDRLP4 4
ADDRGP4 cg+157008
INDIRI4
ASGNI4
...
line 1056
;1055:
;1056:	cmdNum = current - CMD_BACKUP + 1;
ADDRLP4 0
ADDRLP4 476
INDIRI4
CNSTI4 64
...
ASGNI4
line 1057
;1057:	if ( cmdNum < 0 ) // can happen on first spawn
ADDRLP4 0
INDIRI4
CNSTI4 0
GEI4 $794
...
;1061:	// moved = qfalse;
;1062:
;1063:	for ( /* cmdNum = current - CMD_BACKUP + 1 */; cmdNum <= current ; cmdNum++ ) {
ADDRGP4 $794
JUMPV
LABELV $791
line 1065
;1064:		// get the command
;1065:		trap_GetUserCmd( cmdNum, &cg_pmove.cmd );
ADDRLP4 0
INDIRI4
ARGI4
ADDRGP4 cg_pmove+4
...
line 1067
;1066:
;1067:		if ( cgs.pmove_fixed ) {
ADDRGP4 cgs+150076
INDIRI4
CNSTI4 0
EQI4 $796
line 1068
;1068:			PM_UpdateViewAngles( cg_pmove.ps, &cg_pmove.cmd );
ADDRGP4 cg_pmove
INDIRP4
ARGP4
ADDRGP4 cg_pmove+4
...
line 1072
;1070:
;1071:		// don't do anything if the time is before the snapshot player time
;1072:		if ( cg_pmove.cmd.serverTime <= cg.predictedPlayerState.commandTime ) {
ADDRGP4 cg_pmove+4
INDIRI4
ADDRGP4 cg+107636
INDIRI4
...
;1074:		}
;1075:
;1076:		// don't do anything if the command was from a previous map_restart
;1077:		if ( cg_pmove.cmd.serverTime > latestCmd.serverTime ) {
ADDRGP4 cg_pmove+4
INDIRI4
ADDRLP4 488
INDIRI4
...
;1079:		}
;1080:
;1081:		// check for a prediction error from last frame
;1082:		// on a lan, this will often be the exact value
;1083:		// from the snapshot, but on a wan we will have
;1084:		// to predict several commands to get to the point
;1085:		// we want to compare
;1086:		if ( cg.predictedPlayerState.commandTime == oldPlayerState.commandTime ) {
ADDRGP4 cg+107636
INDIRI4
ADDRLP4 8
INDIRI4
...
;1088:			float	len;
;1089:
;1090:			if ( cg.thisFrameTeleport ) {
ADDRGP4 cg+107592
INDIRI4
CNSTI4 0
EQI4 $810
line 1092
;1091:				// a teleport will not cause an error decay
;1092:				VectorClear( cg.predictedError );
ADDRGP4 cg+108852
CNSTF4 0
ASGNF4
ADDRGP4 cg+108852+4
...
EQI4 $818
line 1094
;1094:					CG_Printf( "PredictionTeleport\n" );
ADDRGP4 $821
ARGP4
ADDRGP4 CG_Printf
CALLV
...
LABELV $818
line 1096
;1096:				cg.thisFrameTeleport = qfalse;
ADDRGP4 cg+107592
CNSTI4 0
ASGNI4
line 1099
;1097:
;1098:				// delay prediction for some time or until first server event
;1099:				cg.allowPickupPrediction = cg.time + PICKUP_PREDICTION_DELAY;
ADDRGP4 cg+157024
ADDRGP4 cg+107604
INDIRI4
CNSTI4 200
...
LABELV $810
line 1102
;1101:				vec3_t adjusted, new_angles;
;1102:				CG_AdjustPositionForMover( cg.predictedPlayerState.origin, 
ADDRGP4 cg+107636+20
ARGP4
...
pop
line 1105
;1103:					cg.predictedPlayerState.groundEntityNum, cg.physicsTime, cg.oldTime, adjusted, cg.predictedPlayerState.viewangles, new_angles);
;1104:
;1105:				if ( cg_showmiss.integer ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 0
EQI4 $833
line 1106
;1106:					if ( !VectorCompare( oldPlayerState.origin, adjusted ) ) {
ADDRLP4 8+20
ARGP4
ADDRLP4 560
ARGP4
...
NEI4 $836
line 1107
;1107:						CG_Printf( "prediction error\n" );
ADDRGP4 $839
ARGP4
ADDRGP4 CG_Printf
CALLV
...
LABELV $833
line 1110
;1110:				VectorSubtract( oldPlayerState.origin, adjusted, delta );
ADDRLP4 544
ADDRLP4 8+20
INDIRF4
ADDRLP4 560
...
ASGNF4
line 1111
;1111:				len = VectorLengthSquared( delta );
ADDRLP4 544
ARGP4
ADDRLP4 584
ADDRGP4 VectorLengthSquared
...
ASGNF4
line 1112
;1112:				if ( len > (0.01f * 0.01f) ) {
ADDRLP4 556
INDIRF4
CNSTF4 953267991
LEF4 $849
line 1113
;1113:					if ( cg_showmiss.integer ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 0
EQI4 $851
line 1114
;1114:						CG_Printf( "Prediction miss: %f\n", sqrt( len ) );
ADDRLP4 556
INDIRF4
ARGF4
ADDRLP4 588
...
;1118:						float	f;
;1119:
;1120:						t = cg.time - cg.predictedErrorTime;
ADDRLP4 592
ADDRGP4 cg+107604
INDIRI4
ADDRGP4 cg+108848
...
ASGNI4
line 1121
;1121:						f = ( cg_errorDecay.value - t ) / cg_errorDecay.value;
ADDRLP4 588
...
LABELV $862
line 1125
;1125:						if ( f > 0 && cg_showmiss.integer ) {
ADDRLP4 588
INDIRF4
CNSTF4 0
LEF4 $864
...
EQI4 $864
line 1126
;1126:							CG_Printf("Double prediction decay: %f\n", f);
ADDRGP4 $867
ARGP4
ADDRLP4 588
INDIRF4
...
LABELV $863
line 1128
;1128:						VectorScale( cg.predictedError, f, cg.predictedError );
ADDRGP4 cg+108852
ADDRGP4 cg+108852
INDIRF4
ADDRLP4 588
...
LABELV $855
line 1130
;1130:						VectorClear( cg.predictedError );
ADDRGP4 cg+108852
CNSTF4 0
ASGNF4
ADDRGP4 cg+108852+4
...
LABELV $856
line 1132
;1132:					VectorAdd( delta, cg.predictedError, cg.predictedError );
ADDRGP4 cg+108852
ADDRLP4 544
INDIRF4
ADDRGP4 cg+108852
...
ASGNF4
line 1133
;1133:					cg.predictedErrorTime = cg.oldTime;
ADDRGP4 cg+108848
ADDRGP4 cg+107608
INDIRI4
ASGNI4
...
line 1140
;1137:
;1138:		// don't predict gauntlet firing, which is only supposed to happen
;1139:		// when it actually inflicts damage
;1140:		cg_pmove.gauntletHit = qfalse;
ADDRGP4 cg_pmove+36
CNSTI4 0
ASGNI4
line 1142
;1141:
;1142:		if ( cg_pmove.pmove_fixed ) {
ADDRGP4 cg_pmove+208
INDIRI4
CNSTI4 0
EQI4 $898
line 1143
;1143:			cg_pmove.cmd.serverTime = ((cg_pmove.cmd.serverTime + cg_pmove.pmove_msec-1) / cg_pmove.pmove_msec) * cg_pmove.pmove_msec;
ADDRGP4 cg_pmove+4
ADDRGP4 cg_pmove+4
INDIRI4
ADDRGP4 cg_pmove+212
...
line 1150
;1145:#if 0
;1146:		if ( !cg_optimizePrediction.integer ) {
;1147:			Pmove (&cg_pmove);
;1148:		} else 
;1149:#endif
;1150:		if ( /*cg_optimizePrediction.integer && */ ( cmdNum >= predictCmd || ( stateIndex + 1 ) % NUM_SAVED_STATES == cg.stateHead ) ) {
ADDRLP4 0
INDIRI4
ADDRLP4 484
INDIRI4
...
line 1155
;1153:
;1154:			// add push trigger movement effects
;1155:			CG_TouchTriggerPrediction();
ADDRGP4 CG_TouchTriggerPrediction
CALLV
pop
line 1158
;1156:
;1157:			// check for expired powerups etc.
;1158:			CG_CheckTimers();
...
line 1161
;1159:
;1160:			// record the last predicted command
;1161:			cg.lastPredictedCommand = cmdNum;
ADDRGP4 cg+126112
ADDRLP4 0
INDIRI4
ASGNI4
line 1164
;1162: 
;1163:			// if we haven't run out of space in the saved states queue
;1164:			if( ( stateIndex + 1 ) % NUM_SAVED_STATES != cg.stateHead ) {
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
...
EQI4 $907
line 1167
;1165:				// save the state for the false case ( of cmdNum >= predictCmd )
;1166:				// in later calls to this function
;1167:				cg.savedPmoveStates[ stateIndex ] = *cg_pmove.ps;
ADDRLP4 4
INDIRI4
CNSTI4 468
MULI4
...
LABELV $906
line 1172
;1172:			*cg_pmove.ps = cg.savedPmoveStates[ stateIndex ];
ADDRGP4 cg_pmove
INDIRP4
ADDRLP4 4
INDIRI4
...
ASGNB 468
line 1173
;1173:			stateIndex = ( stateIndex + 1 ) % NUM_SAVED_STATES;
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
...
line 1179
;1178:
;1179:	if ( cg_showmiss.integer > 3 ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 3
LEI4 $917
line 1180
;1180:		CG_Printf( "[%i : %i] ", cg_pmove.cmd.serverTime, cg.time );
ADDRGP4 $920
ARGP4
ADDRGP4 cg_pmove+4
INDIRI4
...
NEI4 $923
line 1184
;1184:		if ( cg_showmiss.integer ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 0
EQI4 $925
line 1185
;1185:			CG_Printf( "not moved\n" );
ADDRGP4 $928
ARGP4
ADDRGP4 CG_Printf
CALLV
...
;1190:	}
;1191:
;1192:	// adjust for the movement of the groundentity
;1193:	CG_AdjustPositionForMover( cg.predictedPlayerState.origin, cg.predictedPlayerState.groundEntityNum, 
ADDRGP4 cg+107636+20
ARGP4
ADDRGP4 cg+107636+68
INDIRI4
...
pop
line 1197
;1194:		cg.physicsTime, cg.time, cg.predictedPlayerState.origin, 
;1195:		cg.predictedPlayerState.viewangles, cg.predictedPlayerState.viewangles );
;1196:
;1197:	if ( cg_showmiss.integer ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 0
EQI4 $941
line 1198
;1198:		if ( cg.predictedPlayerState.eventSequence > oldPlayerState.eventSequence + MAX_PS_EVENTS ) {
...
LEI4 $944
line 1199
;1199:			CG_Printf( "WARNING: dropped event\n" );
ADDRGP4 $949
ARGP4
ADDRGP4 CG_Printf
CALLV
...
line 1204
;1202:
;1203:	// fire events and other transition triggered things
;1204:	CG_TransitionPlayerState( &cg.predictedPlayerState, &oldPlayerState );
ADDRGP4 cg+107636
ARGP4
ADDRLP4 8
ARGP4
...
line 1206
;1205:
;1206:	if ( cg_showmiss.integer ) {
ADDRGP4 cg_showmiss+12
INDIRI4
CNSTI4 0
EQI4 $951
line 1207
;1207:		if ( cg.eventSequence > cg.predictedPlayerState.eventSequence ) {
ADDRGP4 cg+108864
INDIRI4
ADDRGP4 cg+107636+108
INDIRI4
LEI4 $954
line 1208
;1208:			CG_Printf( "WARNING: double event\n" );
ADDRGP4 $959
ARGP4
ADDRGP4 CG_Printf
CALLV
pop
line 1209
;1209:			cg.eventSequence = cg.predictedPlayerState.eventSequence;
ADDRGP4 cg+108864
ADDRGP4 cg+107636+108
INDIRI4
ASGNI4
