...
verified to not have been modified.  A single exception is made for rwaconfig.cfg.  Files
...
When building a pak file, make sure a rwaconfig.cfg isn't present in it,
...
void FS_Restart( int checksumFeed ) {
	...
	if ( Q_stricmp(fs_gamedirvar->string, lastValidGame) && execConfig ) {
		// skip the rwaconfig.cfg if "safe" is on the command line
		if ( !Com_SafeMode() ) {
